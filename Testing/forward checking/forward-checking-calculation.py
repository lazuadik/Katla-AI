...
class CSPWordle:
    ...
    def choices(self, var):
        return sorted(self.domains[var], key=lambda val: val)
    ...
    for target in words:
        ...
        while True:
            ...
            if(guessed_word == target):
                print(guessed_word)
                back_lines.append(back_row+f"{back}\n")
                conf_lines.append(conf_row+f"{conf}\n")
                break
            back_row += f"{back};"
            conf_row += f"{conf};"
            guesser.backtrack_called = 0
            guesser.conflict_count = 0

    open("Testing/forward checking/Result/forward-checking-backtracking.txt", "w").writelines(back_lines)
    open("Testing/forward checking/Result/forward-checking-conflict.txt", "w").writelines(conf_lines)

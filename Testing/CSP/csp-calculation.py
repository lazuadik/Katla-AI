        ...
        try:
            ...
            while True:
                ...
                if(guessed_word == target):
                    print(guessed_word)
                    choose_time_exec.append(choose_time+f"{((choose_end_time-choose_start_time)*1000):.2f}"+'\n')
                    resp_time_exec.append(resp_time+f"{((resp_end_time-resp_start_time)*1000):.2f}"+'\n')
                    freq.append(str(step)+'\n')
                    break
                choose_time += f"{((choose_end_time-choose_start_time)*1000):.2f};"
                resp_time += f"{((resp_end_time-resp_start_time)*1000):.2f};"
        except:
            print("Exception!")
            return 0
        solve_end_time = time.time()
        solve_time_exec.append(f"{((solve_end_time-solve_start_time)*1000):.2f}\n")
    
    open("Testing/CSP/Result/csp-choose.txt", "w").writelines(choose_time_exec)
    open("Testing/CSP/Result/csp-feedback-response.txt", "w").writelines(resp_time_exec)
    open("Testing/CSP/Result/csp-solve.txt", "w").writelines(solve_time_exec)
    open("Testing/CSP/Result/csp-step.txt", "w").writelines(freq)

...
class WordleGuesser:
    ...
    def noor(self, var, assignment):
        return sorted(self.csp.choices(var), key=lambda val: val)
    ...
    def backtrack(self, assignment, assignmentPos):
        ...
        if len(assignment) == 5:
            if self.csp.yellow_state_constraints(assignment):
...
def main():
    words_target = [word.upper() for word in open("Testing/LCV/sample.txt", "r").read().split("\n")]
        ...
        # try:
            ...
            while True:
                ...
                if(guessed_word == target):
                    print(guessed_word)
                    lines.append(row+f"{(end_time-start_time)*1000:.2f}\n")
                    break
                row += f"{(end_time-start_time)*1000:.2f};"
                # guesser.conflict = 0
        # except:
        #     print("Exception!")
        #     return 0
    ...
    open("Testing/LCV/Result/final-convergence.txt", "w").writelines(lines)
